...
print('** AOC_TILES_SCRIPT_DIR:', AOC_TILES_SCRIPT_DIR)
...
def parse_leaderboard(leaderboard_path: Path) -> dict[str, DayScores]:
    ...
    with open(leaderboard_path) as file:
        ...
        assert len(
            matches,
        ) == 1, f"Found {'no' if len(matches) == 0 else 'more than one'} leaderboard?!"
...
def request_leaderboard(year: int) -> dict[str, DayScores]:
    ...
    with open(SESSION_COOKIE_PATH) as cookie_file:
        ...
        data = requests.get(
            PERSONAL_LEADERBOARD_URL.format(
                year=year,
...
def get_alternating_background(languages, both_parts_completed=True, *, stripe_width=20):
    colors = [
        ImageColor.getrgb(extension_to_color[language])
...
def generate_day_tile_image(day: str, year: str, languages: list[str], day_scores: DayScores | None) -> Path:
    ...
    # === Left side ===
    drawer.text(
        (3, -5), 'Day', fill=font_color,
        align='left', font=main_font(20),
    )
    drawer.text(
        (1, -10), str(day), fill=font_color,
    ...
    drawer.text(
        (0, 74), lang_as_str, fill=font_color,
    ...
    for part in (1, 2):
        ...
        if day_scores is not None and time is not None:
            ...
            drawer.text(
                (105, 25 + y), 'time', fill=font_color,
                align='right', font=secondary_font(10),
            )
            drawer.text(
                (105, 35 + y), 'rank', fill=font_color,
                align='right', font=secondary_font(10),
            )
            drawer.text(
                (143, 3 + y), format_time(time),
...
def handle_year(year_path: Path, year: int):
    ...
    days_with_filled_gaps = {
        find_first_number(
            p.name,
        ): p for p in get_paths_matching_regex(year_path, DAY_PATTERN)
    }
    if not CREATE_ALL_DAYS and len(days_with_filled_gaps) == 0:
        print(f'Year {year} is empty!')
        return
    max_day = 25 if CREATE_ALL_DAYS else max(
        *days_with_filled_gaps, *map(int, leaderboard),
    )
